...
# the sum of the yearly_salary
 ...
 # keep only observations of worker-firm that worked in February
 df <- df[month02 == 1]
 ...
 # gen indicator for "real" job : 1) more that 4 month during a calander year 2) yearly earnings more that 150% monthly average wage
 df[,real := 1 * (months >=4 & yearly_salary > 1.5 * mean_salary ) ]
 ...
 df <- df [ , salary_rank := rank(salary)/.N]
